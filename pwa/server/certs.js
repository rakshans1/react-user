...
if (!fs.existsSync(PRIVATE_FOLDER_PATH)) {
  fs.mkdirSync(PRIVATE_FOLDER_PATH);
}
...
getDevelopmentCertificate('react-user', { installCertutil: true })
  .then(({key, cert}) => {
  ...
  process.stderr.write(chalk.red(`   ↪ There was a problem automatically generating X.509 certificates for HTTPS!`));
  process.exit(1);
});
